...
```json
{
  ...
  "retryable": true,
  "request_id": "req_01J9ZK4P2H8X4F2A3B6C7D8E9F",
  "details": {
...
| `request_id` | string | Trace identifier. Mirrors the `X-Request-Id` response header. Include in support cases. |
...
Every response includes an `X-Request-Id` header and the same value in `request_id` on error bodies. Persist it in your client logs and attach it to every support case — it is the fastest path to a root-cause answer.
