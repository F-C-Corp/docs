...
| `idempotency_key_conflict` | Mutating operation endpoints | The `Idempotency-Key` header was reused with a **different** request payload (body or path params). Pick a fresh key for the new request. |
| `idempotency_key_in_progress` | Mutating operation endpoints | A prior request with this `Idempotency-Key` is still being processed. Wait for it to finish before retrying with the same key — the retry will return the cached response of the original. |
...
Idempotency keys, retention windows, and concurrency rules ([`If-Match` / `ETag`](/errors-and-limits#concurrency-rules)) are documented on the [Limits](/errors-and-limits) page. The errors those rules emit are in the tables above: `precondition_required` and `precondition_failed` (concurrency), and `idempotency_key_conflict` and `idempotency_key_in_progress` (idempotency). Omitting `Idempotency-Key` on a mutating operation endpoint returns `400` with `code: "idempotency_key_required"`.
