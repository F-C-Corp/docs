...
The response carries the new `session_id`. Store it — every operation you send on this register until close must reference it.
...
<Steps>
  ...
  <Step title="Ring up the first sale (4.50 EUR)">
    ...
    The response includes `"session_id": "ses_abc123"` even though you did not send it.
  </Step>
