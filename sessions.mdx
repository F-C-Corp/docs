...
The operation `status` is `completed` because session-event operations are final the moment OpenFiskal accepts them, i.e. there is no `PATCH .../complete` step. Don't confuse it with the session's lifecycle state (the [`open` / `closed`](#session-states) flag tracked separately).
