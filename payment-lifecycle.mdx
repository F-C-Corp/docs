...
A return is a **new operation** with `type: return` and `related_operation_id` set to the original sale's operation ID. The return's `payments` array on completion describes the settlement legs that returned money to the customer.
