...
If you complete checkout in your POS before OpenFiskal is reachable again, treat any locally produced receipt as provisional. After the queued completion succeeds, the authoritative fiscal payload is the `fiscal_information` block on the completion response. Reprint or re-render using those verbatim fields — regime validation depends on them (TSE signature, QR data, document number).
