...
All queued operations must be synced and either completed or cancelled before calling `POST /registers/:id/closings`.

<Warning>
  The closing endpoint returns `409 Conflict` if open operations remain on the register. Drain the queue and complete or cancel every operation before triggering the closing.
</Warning>
...
- **Queue drain before closing** — block the daily closing until all queued operations are synced and completed or cancelled.
