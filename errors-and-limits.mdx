...
Forward-looking platform rules — these describe the contract clients should code against; server-side enforcement is not yet live (see [Reserved codes](/errors#reserved-codes-not-yet-emitted)):
...
Until enforcement lands, sending an `Idempotency-Key` is safe and forward-compatible — the API accepts the header but does not currently dedupe on it.
