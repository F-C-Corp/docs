...
Creating a register is cheap — it's just metadata. Creating a fiscal unit is also lightweight — activating it is what provisions the fiscal device and starts billing. This separation lets you:
...
```
created ──► provisioning ──► active ──► decommissioned
                                │
...
- **created**: The fiscal unit record exists but the fiscal device has not been provisioned. No billing occurs. Activate via `POST /fiscal-units/:id/activate` to begin provisioning.
...
<Steps>
  <Step title="Create the register">
    A register is a first-class resource. Set `location_id` to associate it with a location.
  ...
  <Step title="Create a fiscal unit">
    Creating a fiscal unit is lightweight — it creates a record in `created` state without provisioning any fiscal device or incurring costs.
    ...
    The fiscal unit exists but is not yet provisioned — the register still cannot process operations.
  </Step>

  <Step title="Activate the fiscal unit">
    Activating triggers jurisdiction-specific fiscal device initialization and starts billing:
...
```bash
curl "https://api.openfiskal.com/v1/registers/status?location_id=loc_01HXYZ" \
  -H "Authorization: Bearer $API_KEY" \
  -H "OpenFiskal-Organization: org_01HXYZ"
```
...
If a fiscal device fails (TSE hardware fault, RT communication error), the fiscal unit transitions to `error`. Operations on the register will be rejected until the fiscal unit is recovered.
...
The `POST /registers/:id/activate` endpoint is deprecated but continues to work. Under the hood, it creates and auto-activates a fiscal unit, skipping the `created` state. The register response includes deprecated `fiscal_state` and `fiscal_registration` fields derived from the active fiscal unit.
