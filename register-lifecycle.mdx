...
The `POST /registers/:id/activate` endpoint is deprecated but continues to work. Under the hood, it creates a fiscal unit. The register response includes deprecated `fiscal_state` and `fiscal_registration` fields derived from the active fiscal unit.
