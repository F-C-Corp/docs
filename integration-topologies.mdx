...
Your backend server holds a platform credential (`client_id` + `client_secret`), exchanges it for a short-lived Bearer token, and makes all API calls on behalf of registers. The registers themselves never talk to OpenFiskal.
...
1. Your backend calls `POST /auth/token` to get a Bearer token (valid 1 hour, refresh before expiry).
2. When a register reports a sale, your backend calls `POST /registers/:id/sales` with the Bearer token and the `OpenFiskal-Organization` header.
...
- One credential to manage — no per-device key distribution.
- The `client_secret` stays on your server, never on hardware that could be physically compromised.
- Single token works across all organizations and registers.
...
|---|---|---|
| Credential management | One platform credential | One API key per device |
...
Some integrations use both. The backend handles provisioning, fleet management, and monitoring (using platform credentials), while the registers handle real-time fiscal operations directly (using register API keys).

```mermaid
flowchart LR
    B[Backend] -- "Bearer token\n(provisioning, monitoring)" --> O[OpenFiskal API]
...
For device-direct integrations, provisioning involves three steps — all done from your backend using platform credentials:
...
<CardGroup cols={2}>
  <Card title="Authentication" icon="key" href="/auth">
    Credential details, token flows, and the full auth API.
  </Card>
