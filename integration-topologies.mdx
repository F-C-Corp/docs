...
1. During provisioning, your backend creates the register, creates a fiscal unit, and activates it with `issue_register_credential: true`. This returns a one-time register API key.
...
3. The device includes the key in the `X-Register-Api-Key` header for all API calls. No `OpenFiskal-Organization` header is required on device-authenticated requests.
...
For device-direct integrations, provisioning involves four steps — all done from your backend using platform API keys:
...
<Steps>
  ...
  <Step title="Create a fiscal unit">
    ...
    ```json
    {
      ...
      "fiscal_unit": { "id": "fu_01HXYZ", "state": "created" },
      "register_api_key": null,
      "credential_issued": false
    }
    ```
  </Step>
  <Step title="Activate with a register API key">
