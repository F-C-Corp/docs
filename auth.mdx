...
A register API key is issued when creating a fiscal unit by setting `issue_register_credential: true`:

```bash
curl -X POST https://api.openfiskal.com/v1/registers/reg_01ABC/fiscal-units \
  -H "Authorization: Bearer ofk_live_eyJhbGciOiJS..." \
  -H "OpenFiskal-Organization: org_01HXYZ" \
  -H "Content-Type: application/json" \
