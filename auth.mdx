...
A register API key is issued during register activation by setting `issue_register_credential: true`:

```bash
curl -X POST https://api.openfiskal.com/v1/registers/reg_01ABC/activate \
  -H "Authorization: Bearer ofk_live_eyJhbGciOiJS..." \
  -H "OpenFiskal-Organization: org_01HXYZ" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST https://api.openfiskal.com/v1/registers/reg_01ABC/rotate-credential \
  -H "Authorization: Bearer ofk_live_eyJhbGciOiJS..." \
  -H "OpenFiskal-Organization: org_01HXYZ"
```
