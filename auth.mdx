...
description: "How authentication, seller scoping, idempotency, and conditional writes work in the OpenFiskal API."
---
...
For every seller-scoped request, also send:

```bash
-H "X-OpenFiskal-Seller: seller_01HXYZ"
```

This header must contain the OpenFiskal seller ID, not the API key.

Together, a typical seller-scoped request looks like this:
...
Seller-scoped resources include:
...
You do not need `X-OpenFiskal-Seller` when you create, update, delete, or read the seller itself.
...
- Return `403` when the API key is valid but not allowed to access the requested tenant, country, or seller.
