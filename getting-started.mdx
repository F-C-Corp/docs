...
<Steps>
  <Step title="Bootstrap your API key">
  ...
  <Step title="Create a fiscal unit">
    Creating a fiscal unit is lightweight — it creates a record without provisioning the fiscal device or starting billing.
    ...
    The fiscal unit exists in `created` state — the register cannot process operations yet.
  </Step>
  <Step title="Activate the fiscal unit">
    Activating provisions the fiscal security device and starts billing. For Germany, this sets up the TSE and establishes the signature chain.

    ```bash
    curl -X POST https://sandbox.api.openfiskal.com/v1/fiscal-units/fu_01HXYZ/activate \
      -H "Authorization: Bearer ofk_platform_live_abc123..." \
      -H "OpenFiskal-Organization: org_01HXYZ"
    ```
...
<CardGroup cols={2}>
  <Card icon="key" href="/auth" title="Authentication">
    API key types, organization scoping, and key management.
  </Card>
  <Card icon="microchip" href="/register-lifecycle" title="Register Lifecycle">
    Fleet management — commissioning, monitoring, and decommissioning registers.
  </Card>
  <Card icon="cash-register" href="/pos-operation-ingestion" title="POS Operations">
    The full operation lifecycle — start, mutate, complete, cancel.
  </Card>
  <Card icon="database" href="/api-reference/schemas/sale-operation" title="Schemas">
