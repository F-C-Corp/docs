...
<Steps>
  <Step title="Start an operation">
    `POST /operations` is a single-shot create. The body is flat (no `data` wrapper). All monetary fields are decimal strings, not integers. `pretax_amount + tax_amount + tip_amount` must equal `total_amount`. `line_items` is required with at least one entry. `location_id` is not sent — the server derives it from the register.
        ...
        "line_items": [
          {
            ...
            "total_amount": "42.50",
            "taxes": [
              { "name": "MwSt. 7%", "rate": "0.07", "tax_amount": "2.58" }
            ]
          }
        ]
      }'
      ...
      "line_items": [
        {
          ...
          "total_amount": "42.50",
          "taxes": [
            { "id": "lit_01HXYZ", "name": "MwSt. 7%", "rate": "0.07", "tax_amount": "2.58" }
          ]
        }
      ],
      "payments": [],
...
Completed operations cannot be voided — use a `refund` operation instead.
