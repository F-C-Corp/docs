...
<Update label="May 3, 2026" description="Fiscalization API" tags={["Breaking", "Addition"]}>
  ...
  **Breaking — `sku_identifier` is mandatory.** German DSFinV-K cashpoint-closings require a non-empty `lines[i].item.number` per line — the auditor's join key into the merchant's product master. Operations posted without `sku_identifier` on every line item are rejected with `400`.
