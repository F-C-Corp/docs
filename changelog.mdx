...
<Update label="May 7, 2026" description="Fiscalization API" tags={["Addition"]}>
  **Decimal precision validation on `tax_amount`**

  Goods-movement operations (`sale`, `return`, `exchange`) now validate that each `line_items[].taxes[].tax_amount` is consistent with the same line's `total_amount` (gross) and tax `rate`, using the inclusive-tax formula `total_amount × rate_i / (1 + Σ rate_j)`. The divisor sums every rate on the line, so multi-tax stacking (e.g. US-style state + municipal sales tax on the same item) is checked correctly — the same formula collapses to `total_amount × rate / (1 + rate)` for the single-tax case. Comparison is strict at 8 decimal places: the expected value is rounded half-up to 8 dp and compared by numeric equality, so submit `tax_amount` at the precision the math actually produces (e.g. `"15.96638655"` for `total_amount: "100.00"` with `rate: "0.19"`, not `"15.97"`). Mismatches are rejected with `422 tax_amount_precision_invalid`.

  There is no wire-level cap on the number of fractional digits you can send on any decimal-string field. Trailing zeros (`"15.96638655000000"`) and high-precision inputs that are numerically equal to the 8 dp expected value still pass; only inputs whose value differs from the 8 dp expected are rejected.

  See the new [Decimal precision](/decimal-precision) page for worked examples (including the multi-tax stacking case).
</Update>
