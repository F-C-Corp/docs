...
```
       ...
       ─────────►│   open   ├────────────►│ completed  ├────────►│ voided │
                  └─┬──┬──┬─┘              └───────────┘         └────────┘
                    │  │  │                      ▲
           abandon  │  │  │ complete             │ confirmed
                    │  │  │ (async)              │
                    │  │  │   ┌─────────────────┬┘
                    │  │  └──►│pending_payment  ├─ declined/timeout ─►┌────────┐
                    │  │      └─────────────────┘                     │ failed │
                    │  │                                              └────────┘
                    │  └──────────── fault ─────────────────────────────►▲
                    ▼
...
- **open → pending_payment**: The POS submits a completion request with an async payment method (e.g. card terminal). The operation waits for payment confirmation.
- **pending_payment → completed**: The payment provider confirms settlement. The fiscal end event is signed and the fiscal document is returned.
- **pending_payment → failed**: The payment is declined or times out. No fiscal end event is produced.
