...
<Note>
  This differs from `type: return`, where you send the return value as positive amounts and OpenFiskal negates them internally for the fiscal signature. `type: exchange` uses caller-supplied signs throughout — the API passes them through.
</Note>
