...
```
           start ■
             │
           [open] ─── mutate (not fiscal)
           / │ \
  complete ■ │  abandon    fault
         │   │      │        │
...
> **■** = fiscal event (signed by the fiscal security device). While `open`, the operation can be **mutated** freely (add items, discounts, customer data) — mutations are not fiscal events. Terminal states: `completed`, `abandoned`, `failed`, `voided`.
