...
```
start ■ ──► [open] ──► complete ■ ──► [completed] ──► void ■ ──► [voided]
              │
              └──► abandon ──► [abandoned]
```

> **■** = fiscal event (signed by the fiscal security device). While `open`, the operation can be **mutated** freely (add items, discounts, customer data) — mutations are not fiscal events. Terminal states: `completed`, `abandoned`, `voided`.
...
- **completed → voided**: A reversal before the daily closing. A void is a separate fiscal operation that references the original and produces its own fiscal reversal document. After the closing, use a refund instead.

<Info>
  If a fiscalization fault occurs (e.g. the fiscal device is unreachable), the API returns an error — the operation stays in its current state and the POS can retry. There is no `failed` state.
</Info>

```
POS                    OpenFiskal              Fiscal Device
 │                         │                        │
 ├── POST /sales ─────────►│                        │
 │                         ├── sign start ─────────►│
 │                         │◄── signature + counter ┤
 │◄── operation (open) ────┤                        │
 │                         │                        │
 │  PATCH /sales/:id ─────►│  (add items, discounts)│
 │                         │                        │
...
5. **Trigger the daily closing.** At the end of the business day or shift, call `POST /registers/:id/closings`. OpenFiskal generates the fiscal closing report required by the jurisdiction. All open operations must be completed or abandoned first.
