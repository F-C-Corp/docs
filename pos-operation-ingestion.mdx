...
<Steps>
  <Step title="Start">
    ...
    Whether this triggers a fiscal recording depends on the regime. In Germany (TSE), the start is signed immediately — OpenFiskal returns the start signature and transaction counter. **Store these values — they are required on the receipt.** In Austria (RKSV) and Italy (RT), the fiscal device records only at completion; the start call is still required but does not produce fiscal data.
  </Step>
...
- **cancelled**: The POS cancelled the operation before completion. The POS resets.
    ...
    ```bash
    curl -X POST https://api.openfiskal.com/v1/sales/{id}/cancel \
